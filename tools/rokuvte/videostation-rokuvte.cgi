...
def tokenized_external_url(source_url, playlist_url, sid, token):
    ...
    if source_url.startswith(("data:", "skd:")):
        return source_url
...
def rewrite_playlist_line(line, playlist_url, sid, token):
    stripped = line.strip()
    if not stripped:
        return line
    if not stripped.startswith("#"):
        return tokenized_external_url(stripped, playlist_url, sid, token)

    def replace_uri(match):
        return f'URI="{tokenized_external_url(match.group(1), playlist_url, sid, token)}"'
...
def main():
    ...
    lines = []
    for line in playlist.splitlines():
        lines.append(rewrite_playlist_line(line, playlist_url, sid, token))
